...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	 ...
	 * related Places from storage. If this sfGuardUser is new, it will return
		...
		if ($this->collPlaces === null) {
			if ($this->isNew()) {
			   $this->collPlaces = array();
				...
				$criteria->add(PlacePeer::USER_ID, $this->id);
	 ...
	 */
	public function getPlacesJoinCategory($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collPlaces === null) {
			if ($this->isNew()) {
				...
				$this->collPlaces = PlacePeer::doSelectJoinCategory($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
