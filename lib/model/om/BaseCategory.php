...
abstract class BaseCategory extends BaseObject  implements Persistent {
	 ...
	 * related Places from storage. If this Category is new, it will return
		...
		if ($this->collPlaces === null) {
			if ($this->isNew()) {
			   $this->collPlaces = array();
				...
				$criteria->add(PlacePeer::CATEGORY_ID, $this->id);
	...
	public function countPlaces(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collPlaces === null) {
			if ($this->isNew()) {
				...
				$count = PlacePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	 ...
	 */
	public function getPlacesJoinsfGuardUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CategoryPeer::DATABASE_NAME);
		}
		...
		if ($this->collPlaces === null) {
			if ($this->isNew()) {
				...
				$this->collPlaces = PlacePeer::doSelectJoinsfGuardUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
